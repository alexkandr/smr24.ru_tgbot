import re
from aiogram import Router, F
from aiogram.types import Message, CallbackQuery
from aiogram.filters import Command, CommandObject
from aiogram.fsm.context import FSMContext
from routers.utils import OrdersKeyboards, CatalogKeyboards

from models.db_context import DBContext
from models.callback_factory import OrderCallbackFactory

class OrderRouter():
    router = Router()

@OrderRouter.router.message(F.text ==  'Заказы')
async def list_orders(message : Message, db_context : DBContext, user_id = 0):
    user_orders = await db_context.orders.get_by_user_id(message.from_user.id if user_id == 0 else user_id)
    caption = ''
    if user_orders == []:
        caption = 'Здесь пока ничего нет \n Жми -> /catalog'
    else:
        for order in user_orders:
            caption += order.short_info()
    await message.answer_photo(photo=await db_context.images.get_by_name('PhotoArtComplect'),
                               caption=caption)
    
@OrderRouter.router.message(Command(re.compile(r'b_\d{18,20}')))
async def show_order(message : Message, db_context : DBContext, command : CommandObject, state : FSMContext):
    order_id = command.regexp_match.string
    orderitems = await db_context.ordered_items.get_by_order_id(order_id)
    if orderitems == []:
        await message.answer('упс.. кажется, ваш заказ пуст')
        return
    order = await db_context.orders.get_by_id(order_id)
    if order.user_id != message.from_user.id:
        await message.answer('упс.. кажется, этот заказ пуст')
        return
    address = await db_context.addresses.get_by_id(order.address_id)
    await message.answer(text=order.long_info(address.to_string()), reply_markup=await \
                         OrdersKeyboards.get_order(orderitems, order, db_context= db_context))
    await state.update_data(order_id = order_id)
    
@OrderRouter.router.callback_query(OrderCallbackFactory.filter())
async def chosen_address_to_delete(call : CallbackQuery, callback_data : OrderCallbackFactory, state : FSMContext, db_context : DBContext):
    data = callback_data
    match data.action:
        case 'cancel':
            await db_context.orders.update_status(data.order_id, 'отменён', 'не требуется')
            await call.answer('Заказ отменён', show_alert=True)
            await list_orders(call.message, call.from_user.id)
        case 'restore':
            await db_context.orders.update_status(data.order_id, 'восстановлен', 'неоплачен')
            await call.answer('Заказ восстановлен', show_alert=True)
            await list_orders(call.message, call.from_user.id)
        case 'info':
            #todo: callbackfactory не вмещает одновременно order_id и item_id. 
            #поэтому при переходе на страницу товара нет возможности вернуться 
            #на тот же самый заказ
            item = await db_context.items.get_by_id(callback_data.item_id)
            await call.message.edit_text(text=item.message_info(),
                                            reply_markup=OrdersKeyboards.show_item(amount=callback_data.amount, order_id=callback_data.order_id, item_id=item.id))
            await  call.answer()
        case 'back':
            order_id = (await state.get_data())['order_id']
            order = (await db_context.orders.get_by_id(order_id))
            address = await db_context.addresses.get_by_id(order.address_id)
            orderitems = await db_context.ordered_items.get_by_order_id(order.id)
            await call.message.edit_text(text=order.long_info(address.to_string()),
                                            reply_markup= await OrdersKeyboards.get_order(orderitems, order, db_context= db_context))
        case 'sum':
            item = await db_context.items.get_by_id(callback_data.item_id)
            await call.message.answer_photo(photo= item.image, caption=(item.message_info()),reply_markup= CatalogKeyboards.show_item(0, item.id))
            await call.answer()


    
    