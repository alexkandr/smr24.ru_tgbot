...
class ItemsTable:
    ...
    async def get_categories(self, page : int, parent : str = None) -> list[GroupDao]:
        ...
        result = await db.execute(command=query, factory=class_row(GroupDao),
                                  fetch=True)
        data_len = await db.execute(command=len_query, fetch=True)
    ...
    async def get_manufacturers_by_category(self, category : str, available_only : bool = False) -> list[str]:
        ...
        result = [r[0] for r in await db.execute(command=query,
                                  fetch=True)]
